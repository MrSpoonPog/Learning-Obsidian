...
| `.claude/tools/dashboard/lib.mjs` | Shared parsing (`knowledge/*.md` + activity log) used by every dashboard/hook script below |
| `.claude/tools/dashboard/generate.mjs` | Builds the `Frontier` dashboard **HTML** (for the Artifact) *and* `Dashboard.md` (for Obsidian) in one run, from `knowledge/*.md` + `knowledge/_activity.jsonl`. Run + republish the HTML as an Artifact at the end of every `/learn`, `/review`, and `/exam` session — see **Dashboard** below |
| `.claude/tools/dashboard/session-start-summary.mjs` | `SessionStart` hook — prints a one-glance Frontier summary the moment a new Claude Code session opens on this repo |
...
| `Dashboard.md` | Obsidian-native companion to the `Frontier` Artifact — plain markdown/tables/wikilinks, no HTML/iframe, so it renders identically on desktop and mobile Obsidian. Regenerated by `generate.mjs`, same cadence as the HTML |
...
Two views, one generator, always in sync:

- **HTML / "Frontier" Artifact** — live at **https://claude.ai/code/artifact/fd45d4df-e3c7-47d1-b465-a953178825c8**, private, bookmark it on any device. GitHub-style activity heatmap, topic cards, everything visual.
- **`Dashboard.md`** — the same data as plain Obsidian-native markdown, at the repo/vault root. Open it in Obsidian (desktop or mobile) any time — no link, no browser, no login. See **Cross-device access** for making it the first thing you see there.

Both show every topic's node counts (solid/landed/frontier/misconception), next review due date, streak, and total interactions.

**Regenerate both at the end of every `/learn`, `/review`, and `/exam` session:**
1. Append one line to `knowledge/_activity.jsonl`: `{"date":"<today>","topic":"<slug>","type":"learn|review|exam","interactions":<count of AskUserQuestion calls this session>}`
2. `node .claude/tools/dashboard/generate.mjs --out <scratchpad-path>/frontier.html` — this writes **both** the HTML (to `--out`) and `Dashboard.md` (always, to the repo root) in one run.
3. Publish the HTML with the `Artifact` tool, passing `url: "https://claude.ai/code/artifact/fd45d4df-e3c7-47d1-b465-a953178825c8"` so it updates in place rather than creating a new one. `Dashboard.md` needs no publish step — it's just committed and pushed like any other file.

Or just run **`/sync`** — it does exactly the above in one shot, any time, not only at session end.

## Automation

- **`SessionStart` hook** (`.claude/tools/dashboard/session-start-summary.mjs`) — the "open the app, see what's due" moment, like Anki's home screen. Fires automatically at the start of every Claude Code session on this repo and prints a one-glance summary (topics, node counts, next due date, streak, dashboard link) before you've typed anything.
...
- **Dashboard, web** — the `Frontier` Artifact link above works from any device's browser, no login-to-repo needed.
- **Dashboard, in Obsidian** — `Dashboard.md` lives at the vault root and pulls in with everything else via Obsidian Git, so it's just another note. To make it the *first* thing you see on opening Obsidian (the Anki-home-screen effect), install the community **Homepage** plugin and set it to open `Dashboard.md` on launch — optional, not required for the file to exist or sync.
